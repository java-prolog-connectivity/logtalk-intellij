...
public class LogtalkSyntaxHighlighter extends SyntaxHighlighterBase {
    ...
    public static TextAttributesKey[] getTokenHighlights(ExtendedPsiView extendedView) {
        if (extendedView.operationView().isOperator()) {
            return LogtalkSyntaxHighlighter.OPERATOR_KEYS;
        } else if (extendedView.keywordView().isAtomKeyword()) {
            return LogtalkSyntaxHighlighter.KEYWORD_ATOM_KEYS;
        } else if (extendedView.keywordView().isCompoundKeyword()) {
            return LogtalkSyntaxHighlighter.KEYWORD_COMPOUND_NAME_KEYS;
        } else {
            return EMPTY;
        }
    }
